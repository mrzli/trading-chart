...
import { TimeTickTimeframe, TimeTickTimeframeUnit } from '../../../types';
import { getTimeframeUnitRange } from './shared';
import { applyFn } from '@gmjs/apply-function';
import { flatMap, toArray } from '@gmjs/value-transformers';
import {
  getTimeAxisProcessingTimeframesDay,
  getTimeAxisProcessingTimeframesHour,
  getTimeAxisProcessingTimeframesMinute,
  getTimeAxisProcessingTimeframesMonth,
  getTimeAxisProcessingTimeframesYear,
} from './by-time-units';

export function getTimeAxisProcessingTimeframes(
  from: TimeTickTimeframe,
  to: TimeTickTimeframe,
): readonly TimeTickTimeframe[] {
  const timeframeUnitRange = getTimeframeUnitRange(from, to);

  const result = applyFn(
    timeframeUnitRange,
    flatMap((unit) => getTimeAxisProcessingTimeframesForUnit(from, to, unit)),
    toArray(),
  );
...
function getTimeAxisProcessingTimeframesForUnit(
  from: TimeTickTimeframe,
  to: TimeTickTimeframe,
  ...
  switch (unit) {
    case 'm': {
      return getTimeAxisProcessingTimeframesMinute(from, to);
    }
    case 'h': {
      return getTimeAxisProcessingTimeframesHour(from, to);
    }
    case 'D': {
      return getTimeAxisProcessingTimeframesDay(from, to);
    }
    case 'M': {
      return getTimeAxisProcessingTimeframesMonth(from, to);
    }
    case 'Y': {
      return getTimeAxisProcessingTimeframesYear(from, to);
    }
    default: {
