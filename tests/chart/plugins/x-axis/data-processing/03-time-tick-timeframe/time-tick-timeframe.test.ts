...
describe('time-tick-timeframe', () => {
  describe('getMinTimeTickTimeframe()', () => {
    interface Example {
      ...
      readonly expected: TimeTickTimeframe;
    }
    ...
    interface PartialInput {
      ...
      readonly timeframe: TimeAxisInput['timeframe'];
    }

    function createInput(partial: PartialInput): TimeAxisInput {
      const { itemSpan, timeframe } = partial;
    ...
    for (const example of EXAMPLES) {
      it(example.description, () => {
        const actual = getMinTimeTickTimeframe(example.input);
